...
export default function DashboardPage() {
  ...
  const handleUpdateAvatar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userDocRef = studentProfile ? doc(db, 'students', user.uid) : doc(db, 'teachers', user.uid)
      await updateDoc(userDocRef, { photoURL: newAvatarUrl.trim() })
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <div className="bg-primary/10 text-primary px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest flex items-center gap-2">
                  <CreditCard size={14} /> ID: {profile?.studentId || profile?.employeeId || 'N/A'}
