...
export default function AttendancePage() {
  ...
  const studentQrValue = JSON.stringify({
    uid: user.uid,
    studentId: profile?.studentId || profile?.employeeId || 'ADMIN-PREVIEW',
    name: isAdmin ? 'System Administrator' : `${profile?.firstName} ${profile?.lastName}`,
    type: 'techxera-student-id',
    issuedAt: profile?.enrollmentDate || profile?.joinDate || new Date().toISOString()
  })
            ...
            <DialogContent className="sm:max-w-md rounded-[3rem] p-10 border-none glass">
              ...
              <div className="flex flex-col items-center gap-8 py-6">
                ...
                <div className="w-full space-y-2 text-center">
                  ...
                  <p className="text-primary font-black uppercase tracking-widest text-xs">ID: {isAdmin ? 'ROOT' : (profile?.studentId || profile?.employeeId)}</p>
                </div>
